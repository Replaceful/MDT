const mdtApp = new Vue({
    ...
    methods: {
        ...
        closeMDT() {
            $.post('http://ot-mdt/close', JSON.stringify({}));
        },
        getClass(element) {
        ...
        OffenderSearch() {
            if (this.offender_search) {
                ...
                $.post('http://ot-mdt/performOffenderSearch', JSON.stringify({
        ...
        OpenOffenderDetails(id) {
            for (var key in this.offender_results.results) {
                if (id == this.offender_results.results[key].id) {
                    ...
                    $.post('http://ot-mdt/viewOffender', JSON.stringify({
        ...
        SaveOffenderChanges() {
            $.post('http://ot-mdt/saveOffenderChanges', JSON.stringify({
        ...
        ReportSearch() {
            if (this.report_search) {
                ...
                $.post('http://ot-mdt/performReportSearch', JSON.stringify({
        ...
        SubmitNewReport() {
            if (this.report_new.title && this.report_new.char_id && (Object.keys(this.report_new.charges).length > 0) && this.report_new.incident) {
                $.post('http://ot-mdt/submitNewReport', JSON.stringify({
        ...
        OpenOffenderDetailsById(id) {
            $.post('http://ot-mdt/getOffender', JSON.stringify({
                char_id: id
            }));
        ...
        evFile(currentid) {
			$.post('http://ot-mdt/evFile', JSON.stringify({
            id: currentid,
            }));
		},
        ToggleReportEdit() {
        ...
        DeleteSelectedReport() {
            $.post('http://ot-mdt/deleteReport', JSON.stringify({
        ...
        SaveReportEditChanges() {
            $.post('http://ot-mdt/saveReportChanges', JSON.stringify({
                id: this.report_selected.id,
        ...
        VehicleSearch() {
            if (this.vehicle_search) {
                ...
                $.post('http://ot-mdt/vehicleSearch', JSON.stringify({
                    license_plate: this.vehicle_search
                }));
        ...
        OpenVehicleDetails(result) {
            $.post('http://ot-mdt/getVehicle', JSON.stringify({
                vehicle: result
            }));
        ...
        SubmitNewWarrant() {
            ...
            $.post('http://ot-mdt/submitNewWarrant', JSON.stringify({
        ...
        DeleteSelectedWarrant() {
            $.post('http://ot-mdt/deleteWarrant', JSON.stringify({
        ...
        OpenReportById(id) {
            $.post('http://ot-mdt/getReport', JSON.stringify({
                id: id
            }));
        ...
        SaveVehicleEditChanges() {
            $.post('http://ot-mdt/saveVehicleChanges', JSON.stringify({
                license_plate: this.vehicle_selected.license_plate,
    ...
    if (document.readyState === "complete") {
        window.addEventListener('message', function(event) {
            if (event.data.type == "enable") {
                ...
                mdtApp.vehicle_search = mdtApp.vehicle_selected.license_plate;
                ...
                mdtApp.vehicle_results.results = event.data.matches;
                mdtApp.vehicle_results.query = event.data.license_plate;
                mdtApp.vehicle_search = event.data.license_plate;
                mdtApp.vehicle_selected = {
                    license_plate: null,
                    cid: null,
                    owner_id: null,
